...
struct DeltaUIState {
    ...
    int calculationMethodIdx = 1; // Start with Black-Scholes (index 1)
    int steps = 100;
    int sims = 10000;
    ...
    const char* calculationMethods[3] = {"Monte Carlo", "Black-Scholes", "Binomial"};
};
...
void calculatePricing() {
    try {
        // Create trade data with proper constructor
        DPP::TradeData trade(
            static_cast<DPP::OptionExerciseType>(g_state.exerciseTypeIdx),
            static_cast<DPP::OptionPayoffType>(g_state.payoffTypeIdx), 
            g_state.strike,
            g_state.maturity
        );
        
        // Create market data with proper constructor  
        DPP::MarketData market(
            g_state.volatility,
            g_state.underlyingPrice, 
            g_state.interestRate
        );
        
        // Create calculation data for all Greeks
        std::vector<DPP::CalcData> calculations = {
            DPP::CalcData(DPP::Calculation::PV, 100),
            DPP::CalcData(DPP::Calculation::Delta, 100),
            DPP::CalcData(DPP::Calculation::Gamma, 100),
            DPP::CalcData(DPP::Calculation::Vega, 100),
            DPP::CalcData(DPP::Calculation::Rho, 100)
        };
        
        // For now, only implement Black-Scholes
        if (g_state.calculationMethodIdx == 1) { // Black-Scholes
            auto engineResult = DPP::BlackScholesEngine::create(market, trade, calculations);
            if (engineResult.has_value()) {
                auto& engine = engineResult.value();
                engine->run();
                
                if (!engine->hasAnyErrors()) {
                    // Extract results
                    for (const auto& [calc, result] : engine->m_results) {
                        if (result.has_value()) {
                            switch (calc) {
                                case DPP::Calculation::PV:
                                    g_state.pv = result.value();
                                    break;
                                case DPP::Calculation::Delta:
                                    g_state.delta = result.value();
                                    break;
                                case DPP::Calculation::Gamma:
                                    g_state.gamma = result.value();
                                    break;
                                case DPP::Calculation::Vega:
                                    g_state.vega = result.value();
                                    break;
                                case DPP::Calculation::Rho:
                                    g_state.rho = result.value();
                                    break;
                                default:
                                    break;
                            }
                    ...
                    g_state.errorMessage = engine->getAggregatedErrors();
                    g_state.hasResults = false;
                }
            } else {
                g_state.errorMessage = engineResult.error();
                g_state.hasResults = false;
            }
        } else {
...
void renderCalcParams() {
    if (ImGui::CollapsingHeader("Calculation", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (g_state.calculationMethodIdx != 1) { // Not Black-Scholes
